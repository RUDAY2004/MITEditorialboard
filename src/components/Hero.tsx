    ...
    <section className="relative w-full h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-5xl mx-auto px-4 pt-4 pb-16">
        <h1 className="heading-1 mb-8 text-white">
          <span className="typing-container">
            <TypingAnimation 
              ...
              typingSpeed={100} 
              delay={500}
              className="shimmer-text text-5xl md:text-6xl lg:text-7xl tracking-wide"
            />
          </span>
        </h1>
        
        <p className={`text-lg md:text-xl text-white/90 max-w-3xl mx-auto mb-12 transition-opacity duration-1000 ${showFullText ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className={`transition-all duration-1000 transform ${showFullText ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <button className="bg-white hover:bg-blue-100 text-[#1a1f2c] font-semibold py-3 px-8 rounded-full transition-all duration-300 transform hover:scale-105" onClick={scrollToSection}>
            Explore
          </button>
        </div>
      </div>
